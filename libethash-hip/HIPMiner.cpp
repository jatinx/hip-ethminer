...
#define hiplog clog(HIPChannel)

HIPMiner::HIPMiner(unsigned _index, CUSettings _settings, DeviceDescriptor& _device)
  : Miner("hip-", _index),
...
bool HIPMiner::initDevice()
{
    ...
    try
    {
        HIP_SAFE_CALL(hipSetDevice(m_deviceDescriptor.cuDeviceIndex));
        HIP_SAFE_CALL(hipDeviceReset());
    }
    catch (const hip_runtime_error& ec)
    {
        hiplog << "Could not set HIP device on Pci Id " << m_deviceDescriptor.uniqueId
               << " Error : " << ec.what();
...
bool HIPMiner::initEpoch_internal()
{
    ...
    try
    {
            ...
            m_allocated_memory_light_cache < m_epochContext.lightSize)
        {
            ...
            // TODO Find if its necessary
            // HIP_SAFE_CALL(hipDeviceReset());
            // HIP_SAFE_CALL(cudaSetDeviceFlags(m_settings.schedule));
            // HIP_SAFE_CALL(cudaDeviceSetCacheConfig(cudaFuncCachePreferL1));
            ...
            m_allocated_memory_light_cache = m_epochContext.lightSize;
            HIP_SAFE_CALL(hipMalloc(reinterpret_cast<void**>(&dag), m_epochContext.dagSize));
    ...
    {
        hiplog << "Unexpected error " << ec.what() << " on HIP device "
...
void HIPMiner::workLoop()
{
    ...
    try
    {
        ...
        // HIP_SAFE_CALL(cudaDeviceReset());
    }
    catch (hip_runtime_error const& _e)
    {
...
int HIPMiner::getNumDevices()
{
    ...
    if (err == hipErrorInsufficientDriver)
    {
        ...
        if (driverVersion == 0)
            std::cerr << "HIP Error : No HIP driver found" << std::endl;
        else
            std::cerr << "HIP Error : Insufficient HIP driver " << std::to_string(driverVersion)
                      << std::endl;
    }
    else
    {
...
void HIPMiner::enumDevices(std::map<string, DeviceDescriptor>& _DevicesCollection)
{
    ...
    for (int i = 0; i < numDevices; i++)
    {
        ...
        try
        {
            ...
            HIP_SAFE_CALL(hipMemGetInfo(&freeMem, &totalMem));
        ...
        catch (const hip_runtime_error& _e)
        {
    ...
    uint8_t const* header, uint64_t target, uint64_t start_nonce, const dev::eth::WorkPackage& w)
{
    ...
    while (!done)
    {
        ...
        // This inner loop will process each hip stream individually
        for (current_index = 0; current_index < m_settings.streams;
             current_index++, start_nonce += m_batch_size)
        {
