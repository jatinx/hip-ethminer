...
void ethash_generate_dag(
    uint64_t dag_size, uint32_t gridSize, uint32_t blockSize, hipStream_t stream)
{
...
{
    HIP_SAFE_CALL(hipMemcpyToSymbol(d_dag, &_dag, sizeof(hash128_t*)));
    HIP_SAFE_CALL(hipMemcpyToSymbol(d_dag_size, &_dag_size, sizeof(uint32_t)));
    HIP_SAFE_CALL(hipMemcpyToSymbol(d_light, &_light, sizeof(hash64_t*)));
    HIP_SAFE_CALL(hipMemcpyToSymbol(d_light_size, &_light_size, sizeof(uint32_t)));
}
...
void get_constants(hash128_t** _dag, uint32_t* _dag_size, hash64_t** _light, uint32_t* _light_size)
{
    ...
    */
    ...
    {
        hash128_t* _d;
        HIP_SAFE_CALL(hipMemcpyFromSymbol(&_d, d_dag, sizeof(hash128_t*)));
        *_dag = _d;
    }
    if (_dag_size)
    {
        uint32_t _ds;
        HIP_SAFE_CALL(hipMemcpyFromSymbol(&_ds, d_dag_size, sizeof(uint32_t)));
        *_dag_size = _ds;
    }
    if (_light)
    {
        hash64_t* _l;
        HIP_SAFE_CALL(hipMemcpyFromSymbol(&_l, d_light, sizeof(hash64_t*)));
        *_light = _l;
    }
    if (_light_size)
    {
        uint32_t _ls;
        HIP_SAFE_CALL(hipMemcpyFromSymbol(&_ls, d_light_size, sizeof(uint32_t)));
        *_light_size = _ls;
    }
}
...
{
    HIP_SAFE_CALL(hipMemcpyToSymbol(d_header, &_header, sizeof(hash32_t)));
}

void set_target(uint64_t _target)
{
    HIP_SAFE_CALL(hipMemcpyToSymbol(d_target, &_target, sizeof(uint64_t)));
}
